...
Bodies are capped at 512 KB while streaming, with at most three materials and 256,000 UTF-8 bytes of serialized draft input, including source text, metadata, and JSON formatting. The app’s writing instructions have a separate role and do not reduce that input allowance. Oversized drafts are rejected before reserving quota or calling a provider, with the measured size and directions for reducing the selected input; no excerpts are silently cut. Serialized material arrays use the request-body cap so JSON escaping and citation metadata do not reduce the individual excerpt allowance. TXT/HTML uploads are capped at 128 KB each. PDF and Word (.docx) documents up to 25 MB are parsed as raw text in a browser worker with a 30-second timeout, a 250-page PDF limit, a 500,000-character extraction limit, and declared ZIP expansion checks for DOCX. The user reviews the text and selects PDF pages or edits a passage; each document selection is capped at 16,000 UTF-8 bytes, independently validated by the server. Original PDF/DOCX bytes stay on the device and are never stored or sent to Vercel or an AI provider. Raw PDF/DOCX uploads to the API are rejected. Document HTML, scripts, and links are not rendered or executed.

Output is capped at 6,000 tokens for papers/revisions and 2,000 for other formats by default. `AI_MAX_OUTPUT_TOKENS` lowers the overall ceiling; `AI_ALLOWED_MODELS` optionally restricts models. OpenAI automatic retries are disabled. Provider requests time out after 60 seconds; a timed-out request can still incur provider charges. Batch replies stop when authentication, quota, or configuration checks fail.
