...
6. **Missing material.** Try an unsupported PDF/Word upload and an empty TXT file. The app should explain the problem before calling a model. A pasted URL alone must not be described as a source the model has read.
